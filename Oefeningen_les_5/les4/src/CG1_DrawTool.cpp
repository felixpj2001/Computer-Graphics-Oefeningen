...
CG1_DrawTool::CG1_DrawTool()
{
    ...
    // LoadFromFile("triangle.txt");
    LoadFromFile((char *)"bezier.txt");
}
...
void CG1_DrawTool::DrawData()
{
    ...
    // Draw3DLine(CG1_4DVector(0, 0, 0), CG1_4DVector(100, 100, 0), RGB_Color(1.0f, 0.0f, 0.0f));

    // DrawPolygonFromList(0, RGB_Color(1.0f, 0.0f, 0.0f));
    // DrawObjectFromList(0, RGB_Color(1.0f, 0.0f, 0.0f));
    for (int i = 0; i < lVertices.m_vertices.size() - 1; ++i)
    {
        Object_Vertex vertex1 = lVertices.m_vertices[i];
        Object_Vertex vertex2 = lVertices.m_vertices[i + 1];
        CG1_4DVector vector1(vertex1.getX(), vertex1.getY(), vertex1.getZ());
        CG1_4DVector vector2(vertex2.getX(), vertex2.getY(), vertex2.getZ());
        Draw3DLine(vector1, vector2, RGB_Color(1.0f, 0.0f, 0.0f));
    }
    DrawBezierFromList(0, RGB_Color(1.0f, 0.0f, 0.0f));
...
void CG1_DrawTool::DrawClipRectangle(RGB_Color color)
{
    ...
    DrawMidPointLine(CG1_Line(ClipRight, ClipBottom, ClipLeft, ClipBottom),
                     color);
...
void CG1_DrawTool::DrawMidPointLine(CG1_Line Line, RGB_Color color)
{
    ...
    int dx, dy, IncrementToE, IncrementToN, IncrementToS, IncrementToSE,
        IncrementToNE, d, x, y;
    ...
    if (m >= -1 && m <= 1)
    {
        if (m >= 0)
        {
            ...
            IncrementToNE =
                (dy - dx) * 2; // increment voor d, na verplaatsing naar NE
        ...
        else if (m < 0)
        {
            ...
            IncrementToE = dy * 2; // increment voor d, na verplaatsing naar E
            IncrementToSE =
                (dy + dx) * 2; // increment voor d, na verplaatsing naar SE
...
void CG1_DrawTool::DrawMidPointCircle(int Midx, int Midy, int radius,
                                      RGB_Color color)
{
    int d, x, y;
...
void CG1_DrawTool::DrawSecondOrderMidPointCircle(int Midx, int Midy, int radius,
                                                 RGB_Color color)
{
...
void CG1_DrawTool::DrawAllCirclePoints(int Midx, int Midy, int x, int y,
                                       RGB_Color color)
{
...
void CG1_DrawTool::FillPolygon(CG1_2DPolygon *Polygon, RGB_Color color)
{
    if (Polygon->GetSize() > 0)
    {
        ...
        while (y <= MaximumY)
        {
            ...
            for (AET.ActiveEdgeIterator = AET.ActiveEdges.begin();
                 AET.ActiveEdgeIterator != AET.ActiveEdges.end();
            ...
            AET.RemoveObsoleteEdges(
                y); // zorgt ervoor dat ymax niet mee in de pariteit voorkomt
...
CG1_OutCode CG1_DrawTool::CompOutCode(float x, float y)
{
    ...
    code.all = 0;
    // all: 4 bits ->     top = 8   ---   bottom = 4   ---   right = 2   --- left
    // = 1
    code.top = 0;
    code.bottom = 0;
    code.left = 0;
...
bool CG1_DrawTool::CohenSutherLandClip(CG1_Line *Line)
{
    ...
    do
    {
        ...
        {
            if (OutCode0.all !=
                0) // een buiten de cliprectangle liggend punt (all != 0) verbeteren
            {
                OutCodeOut = OutCode0;
            }
            else
...
bool CG1_DrawTool::CyrusBeckClip(CG1_Line *Line)
{
    ...
    {
        if (x0 - x1 <= 0.001 && x0 - x1 >= -0.001 && y0 - y1 <= 0.001 &&
            y0 - y1 >= -0.001)
        {
        ...
        else
        {
            ...
            for (int i = 0; i < 4; i++) // vier kanten:		0 = onder | 1 = rechts
                                        // | 2 = boven | 3 = links
            {
                switch (i)
                {
            ...
            if (tE < tL)
            {
                ...
                Line->SetData((int)PtE.GetX(), (int)PtE.GetY(), (int)PtL.GetX(),
                              (int)PtL.GetY());
                return true;
            }
            else
...
void CG1_DrawTool::DrawPolygon(CG1_2DPolygon *Polygon, RGB_Color color)
{
    ...
    for (int i = 1; i < NrOfPoints; i++)
    {
        ...
        CurrentEdge.SetData(FirstPoint.GetX(), FirstPoint.GetY(),
    ...
    CG1_Line CurrentEdge;
    CurrentEdge.SetData(FirstPoint.GetX(), FirstPoint.GetY(), SecondPoint.GetX(),
                        SecondPoint.GetY());
    DrawMidPointLine(CurrentEdge, color);
}
...
void CG1_DrawTool::SutherlandHodgemanPolygonClip(CG1_2DPolygon *Polygon)
{
    ...
    if (OldPolygon->GetSize() > 1)
    {
        ...
        for (EdgeNr = 0; EdgeNr < 4; EdgeNr++)
        {
            ...
            first = OldPolygon->GetPoint(OldPolygon->GetSize() -
                                         1); // initieel de laatste

            for (int i = 0; i < OldPolygon->GetSize(); i++) // test alle old points
            {
                second = OldPolygon->GetPoint(i);

                if (Inside(first, EdgeNr, Value) &&
                    Inside(second, EdgeNr, Value)) // case 1
                {
                    // NewPolygon updaten
...
CG1_2DVector CG1_DrawTool::GetIntersection(CG1_2DVector first,
                                           CG1_2DVector second, int EdgeNr,
                                           int Value)
{
    float x0 = first.GetX();
    float y0 = first.GetY();
...
void CG1_DrawTool::Draw3DLine(CG1_4DVector start, CG1_4DVector end,
                              RGB_Color color)
{
    ...
    CG1_Line The_Line = CG1_Line((int)start.GetX(), (int)start.GetY(),
...
void CG1_DrawTool::PushMatrix() { MatrixStack.push(ModelViewMatrix); }
    ...
    if (File != NULL)
    {
        ...
        while (!feof(File))
        {
            ...
            case 'v':
            {
                ...
                printf("%c: (%d) %5.2lf %5.2lf %5.2lf\n", DataType, Vertex_index, X, Y,
                       Z);
                fgets(Line, 500, File);
                lVertices.m_vertices.push_back(Object_Vertex(Vertex_index, X, Y, Z));
                break;
            }
            case 'p':
            {
                ...
                Object_Polygon temp(Polygon_index);
                ...
                while (SpacePos != -1)
                {
                    ...
                    temp.m_vertices.push_back(Vertex_index);
                }
                printf("\n");
                fgets(Line, 500, File);
                lPolygons.m_polygons.push_back(temp);
                break;
            }
            case 'o':
            {
                ...
                Object_Object temp(Object_index);
                ...
                while (SpacePos != -1)
                {
                    ...
                    temp.m_polygons.push_back(Polygon_index);
                }
                printf("\n");
                fgets(Line, 500, File);
                lObjects.m_objects.push_back(temp);
                break;
            }
            case 'b':
            {
                int Bezier_index, ittype, itsteps;
                sscanf(Line, "%c %d %d %d", &DataType, &Bezier_index, &ittype,
                       &itsteps);
                printf("%c: (%d) -> Method: %d, Steps: %d\n", DataType, Bezier_index,
                       ittype, itsteps);
                Object_Bezier temp;
                temp.setMethod(Iteration_Method(ittype));
                temp.setNrOfSteps(itsteps);
                std::string MyString(Line);
                int Vertex_index, SpacePos;
                ...
                while (SpacePos != -1)
                {
                    ...
                    printf("%d ", Vertex_index);
                    SpacePos = MyString.find(' ');
                    Object_Vertex vertex = lVertices.m_vertices[Vertex_index];
                    temp.addVertex(Vertex_index, CG1_4DVector(vertex.getX(), vertex.getY(), vertex.getZ()));
                }
                lBeziers.AddBezier(temp);
                printf("\n");
                fgets(Line, 500, File);
                break;
            }
        ...
        printf("=> %lu objects added, ", lObjects.m_objects.size());
        printf("containing %lu polygons, ", lPolygons.m_polygons.size());
        printf("using %lu vertices.\n", lVertices.m_vertices.size());
...
{
    Object_Object object = lObjects.m_objects[index];
    for (int i = 0; i < object.m_polygons.size(); ++i)
    {
        DrawPolygonFromList(object.m_polygons[i], color);
    }
}
...
{
    Object_Polygon polygon = lPolygons.m_polygons[index];
    int size = polygon.m_vertices.size();
    for (int i = 0; i < size; ++i)
    {
        int id1 = polygon.m_vertices[i % size];
        int id2 = polygon.m_vertices[(i + 1) % size];
        Object_Vertex vertex1 = lVertices.m_vertices[id1];
        Object_Vertex vertex2 = lVertices.m_vertices[id2];
        CG1_4DVector vector1(vertex1.getX(), vertex1.getY(), vertex1.getZ());
        CG1_4DVector vector2(vertex2.getX(), vertex2.getY(), vertex2.getZ());
        Draw3DLine(vector1, vector2, color);
    }
}
...
{
    Object_Bezier *bezier = lBeziers.GetBezier(index);
    bezier->iterate();
    CG1_Bezier *b = bezier->getBezier();
    for (int i = 0; i < b->getNrOfPoints() - 1; i++)
    {
        CG1_4DVector vector1 = b->getPoint(i);
        CG1_4DVector vector2 = b->getPoint(i + 1);
        Draw3DLine(vector1, vector2, RGB_Color(1.0f, 0.0f, 0.0f));
    }
}
...
void CG1_DrawTool::DrawPolygonsUsingZBuffer()
{
    ZBuffer zbuffer;
    zbuffer.Initialize(ClipRight - ClipLeft, ClipTop - ClipBottom, -1000);

    int pz;
    for (int y = -ClipBottom; y < ClipTop; ++y)
    {
        for (int x = ClipLeft; x < ClipRight; ++x)
        {
            canvas->putPixel(x, y, RGB_Color(1.0f, 1.0f, 1.0f));
            zbuffer.SetValue(x, y, -1000);
        }
    }

    for (int i = 0; i < lPolygons.m_polygons.size(); ++i)
    {
        Object_Polygon p = lPolygons.m_polygons[i];
    }
}